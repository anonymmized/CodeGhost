...
std::vector<std::string> Notifier::pollApprovals(Logger& logger) {
    ...
    auto res = client.Get("/approvals");
    if (!res || res->status != 200) {
        logger.log(LOG_WARN, "Notifier: failed to poll approvals");
        return {};
    }
    ...
    try {
        ...
        logger.log(LOG_WARN, "Notifier: failed to parse approvals response");
    }
